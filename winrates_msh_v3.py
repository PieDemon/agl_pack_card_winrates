...
def compute_pure_match_history_timeline():
    """
    Parses the Matches tab chronologically. Reconstructs running records 
    and returns absolute final records based ONLY on true played matches.
    """
...
def main():
    ...
    for player_name, profile in scraped_data.items():
        ...
        # Build pure record boundaries across pack transitions
        ...
        # Isolate valid Marvel cards inside this player's entire footprint
        ...
        # Step through time to record performance between pack intervals
        for card in all_discovered_msh_cards:
            ...
            for i, col_label in enumerate(ordered_packs):
                ...
                # MULTIPLICATIVE WEIGHT: scale results by current volume owned
    ...
    for card_name, stats in card_performance.items():
        ...
        smoothed_winrate = ((weighted_wins + pseudo_wins_added) / (weighted_matches + SMOOTHING_MATCHES)) * 100
        
        final_rankings.append({
            "Card Name": card_name,
            "Bayes Smoothed Winrate %": round(smoothed_winrate, 2),
            "Raw Weighted Winrate %": round(raw_weighted_winrate, 2),
    ...
    print(f"\n--- PURIFIED MARVEL SUPER HEROES SCOREBOARD ---")
    print(f"{'RANK':<5} | {'CARD NAME':<35} | {'BAYES WR':<10} | {'WEIGHTED WR':<12} | {'WEIGHTED RECORD':<16} | {'POOLS'}")
    print("-" * 103)
    for rank, item in enumerate(sorted_rankings[:25], 1):
        bayes_str = f"{item['Bayes Smoothed Winrate %']}%"
        raw_str = f"{item['Raw Weighted Winrate %']}%"
        print(f"{rank:<5} | {item['Card Name']:<35} | {bayes_str:<10} | {raw_str:<12} | {item['Weighted Record']:<16} | {item['Pools Opened In']}")

    with open(OUTPUT_CSV, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=[
            "Card Name", "Bayes Smoothed Winrate %", "Raw Weighted Winrate %", 
            "Weighted Record", "Total Weighted Matches", "Pools Opened In"
        ])
        writer.writeheader()
        writer.writerows(sorted_rankings)

    print(f"\n[ COMPLETE ] Pure weighted analysis saved to '{OUTPUT_CSV}'")
