...
def extract_hyperlink_target(cell):
    """
    Aggressively extracts a URL string, now including regex pattern matching 
    to extract the IFERROR fallback URL embedded inside Google Sheets dummy formulas.
    """
    ...
    # Target and pull any standard web URLs out of formulas or raw text values
    links = re.findall(r'https?://[^\s"\')]+', cell_str)
    if links:
        # Returns the first valid link string discovered in the cell data
...
def main():
    ...
    # Read headers row 1 to map indices dynamically
    headers = [ws.cell(row=1, column=c).value for c in range(1, ws.max_column + 1)]
    headers_clean = [str(h).strip().upper() if h else "" for h in headers]
    
    # Find the identification index column dynamically (e.g., Column 4)
    ...
    for row_idx in range(2, ws.max_row + 1):
        # Read the player ID name directly out of your dynamic identification index column
        player_id = clean_player_name(ws.cell(row=row_idx, column=id_col_idx).value)
        if not player_id or player_id.lower() in ['nan', 'none', '']:
            continue

        # Look up true final records from the matches timeline log map
        ...
        for col_idx in range(1, ws.max_column + 1):
            ...
            if col_name_clean == "STARTING POOL" or col_name_clean.startswith("PACKS ") or col_name_clean.startswith("PACK "):
                ...
                if link_url and "sealeddeck.tech" in link_url:
                    ...
                    # Standardize names to singular 'PACK X' keys for cleaner analysis down-stream
    ...
    # Ensure output directory paths exist safely
