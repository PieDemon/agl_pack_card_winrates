...
def main():
    ...
    for copy_name, stats in card_performance.items():
        # FIX: Check explicitly to isolate ONLY the first copy of each card
        if not copy_name.endswith(" (Copy 1)"):
            continue

        # Strip the duplicate tracking suffix out so names display perfectly
        clean_display_name = copy_name.replace(" (Copy 1)", "")
    ...
    print(f"--- OFFICIAL MARVEL SUPER HEROES SCOREBOARD (FIRST COPIES ONLY) ---")
    print(f"{'RANK':<5} | {'CARD NAME':<35} | {'BAYES WR':<10} | {'RAW WR':<10} | {'RECORD':<10} | {'POOLS'}")
    print("-" * 85)
    for rank, item in enumerate(sorted_rankings[:25], 1):
        ...
        print(f"{rank:<5} | {item['Card Name']:<35} | {bayes_str:<10} | {raw_str:<10} | {item['Combined Post-Acquisition Record']:<10} | {item['Pools Opened In']}")
    ...
    print(f"\n[ COMPLETE ] Unique card analysis compiled cleanly into '{OUTPUT_CSV}'")
