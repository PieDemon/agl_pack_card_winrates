...
def main():
    ...
    for player_name, profile in scraped_data.items():
        ...
        for col_label in ordered_packs:
            ...
            for c in pools[json_key].get("cards", []):
                if c.strip().lower() in sos_whitelist:
                    all_discovered_sos_cards.add(c.strip().lower()) #my edit, added .lower()

        # FIX: Loop through verified cards to add the unique global pool count first
        for card in all_discovered_sos_cards:
            ...
            for i, col_label in enumerate(ordered_packs):
                ...
                opened_in_this_pack = sum(1 for c in pack_cards if c.strip().lower() == card) # my second edit added lower()
    ...
    for card_name, stats in card_performance.items():
        ...
        final_rankings.append({
            "Card Name": card_name.title(), #my edit added .title()
    ...
    # FIX: Widened padding to 50 characters for card names, scaled headers and row string layouts
    print(f"\n--- OFFICIAL SOS CARD SCOREBOARD (RAW WEIGHTED MULTIPLIERS) ---")
    print(f"{'RANK':<5} | {'CARD NAME':<50} | {'WEIGHTED WR':<14} | {'WEIGHTED RECORD':<16} | {'POOLS'}")
    print("-" * 103)
    for rank, item in enumerate(sorted_rankings[:285], 1):
